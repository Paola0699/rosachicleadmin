...
function Balance() {
    ...
    if (startDate && finalDate) {
      ...
      setGeneralOutcome(
        getTotal(
          allOutcomes.filter((outcome) => {
            return (
              outcome.outcomeKind === "Gasto General" &&
              outcome.status === "Autorizado"
            );
          })
        )
      );
      setOperativeOutcome(
        getTotal(
          allOutcomes.filter((outcome) => {
            return (
              outcome.outcomeKind === "Gasto Operativo" &&
              outcome.status === "Autorizado"
            );
          })
        )
      );
      setAdministrativeOutcome(
        getTotal(
          allOutcomes.filter((outcome) => {
            return (
              outcome.outcomeKind === "Gasto Administrativo" &&
              outcome.status === "Autorizado"
            );
          })
        )
      );
      const tempOtherIncomes = getTotal(
        allOutcomes.filter((outcome) => outcome.kind === "Ingreso")
      );
      ...
      const [totalGeneralSales, externTotals, totalIncomes] =
        totalGeneralSalesAndTotalSalesByCategory(allIncomes, externCategories);
  ...
  const totalGeneralSalesAndTotalSalesByCategory = (
    allIncomes,
    externCategories
  ) => {
    let allProducts = [];
    allIncomes.forEach((order) => {
      order.products.forEach((product) => {
        if (!externCategories.some((cat) => cat.name === product.category))
          ...
          const i = externCategories
            .map((c) => c.name)
            .findIndex((c) => c === product.category);
          externCategories[i].productsSold
            ? externCategories[i].productsSold.push(product)
            : (externCategories[i].productsSold = [product]);
        }
      });
    });
    ...
    externCategories.forEach((c) => {
      if (c.productsSold) c.total = c.productsSold.reduce(reducer, 0);
      else c.total = 0;
    });

    const salesNotExtern = allProducts.reduce(reducer, 0);
    const getSumOfTotals = (accumulator, category) =>
      accumulator + category.total;
    const totalIncomes =
      salesNotExtern + externCategories.reduce(getSumOfTotals, 0);
            ...
            >
              <tbody>
                <tr style={{ textAlign: "center" }}>
                  <th colSpan="2" className="is-success">
                    GASTOS
                  </th>
                ...
                <tr
                  style={{
                    textAlign: "center",
                    backgroundColor: "rgba(159, 214, 248, 0.1)",
                  }}
                >
